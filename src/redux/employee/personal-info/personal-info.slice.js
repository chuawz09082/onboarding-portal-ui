...
const personalInfoSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      editEmergencyContactSectionThunk.fulfilled,
      (state, action) => {
        console.log("updated contact information");
        state.status = "successful";
      }
    ...
    builder.addCase(
      editEmergencyContactSectionThunk.rejected,
      (state, action) => {
        console.log("failed to update contact information");
        console.log(action.error.message);
        state.status = "failed";
    ...
    builder.addCase(getPersonalInfoByIdsThunk.pending, (state, action) => {});

    builder.addCase(getPersonalInfoByIdsThunk.fulfilled, (state, action) => {
      state.status = "successful";
      console.log(action.payload);
      state.employeesData = action.payload;
    });
    builder.addCase(getPersonalInfoByIdsThunk.rejected, (state, action) => {
      state.status = "failed";
      state.error = action.error.message;
    });
  },
